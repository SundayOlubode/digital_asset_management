...
contract DigitalAssetManagement {
    ...
    struct DigitalAsset {
        ...
        string name;
        string description;
        string ipfsCid;       // IPFS Content Identifier for artwork retrieval
        string assetHash;     // Cryptographic hash of the asset for verification
    ...
    mapping(string => bool) private registeredCids;
    
    // Events for auditing and monitoring
    event AssetRegistered(uint256 indexed assetId, address indexed owner, string ipfsCid, string assetHash, uint256 timestamp);
     ...
     * @param _ipfsCid IPFS Content Identifier for the artwork
     * @param _assetHash Cryptographic hash of the asset for verification
    ...
    function registerAsset(
        ...
        require(!registeredCids[_ipfsCid], "Asset with this IPFS CID already exists");
        ...
        registeredCids[_ipfsCid] = true;
        
        // Emit event
        emit AssetRegistered(newAssetId, msg.sender, _ipfsCid, _assetHash, block.timestamp);
     ...
     * @return ipfsCid IPFS Content Identifier for asset retrieval
    ...
    /**
     * @dev Checks if an IPFS CID is already registered
     * @param _ipfsCid IPFS CID to check
     * @return isRegistered Whether the IPFS CID is already registered
     */
    function isCidRegistered(string memory _ipfsCid) public view returns (bool) {
        return registeredCids[_ipfsCid];
    }
